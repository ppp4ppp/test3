...
async function httpRequest(req, res) {
  ...
  // Handle GET request for vynetconfig.json
  if (req.method === 'GET' && req.url === '/vynetconfig.json') {
    const configPath = path.join(process.cwd(), 'vynetconfig.json');
    let configData = {};
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        log.error('vynetconfig.json read error:', err.message);
      }
    }
    ...
    res.end(JSON.stringify(configData), 'utf-8');
    log.data(`${req.method}/${req.httpVersion}`, res.statusCode, 'vynetconfig.json', req.url, ip);
    return;
  }

  // Handle POST request for vynetconfig.json
  if (req.method === 'POST' && req.url === '/vynetconfig.json') {
    let body = '';
    req.on('data', (chunk) => {
      body += chunk.toString();
    });
    req.on('end', () => {
      try {
        ...
        res.end(JSON.stringify(configData), 'utf-8');
        log.data(`${req.method}/${req.httpVersion}`, res.statusCode, 'vynetconfig.json saved', body.length, req.url, ip);
      } catch (err) {
        res.writeHead(400, { 'Content-Type': 'application/json; charset=utf-8' });
        res.end(JSON.stringify({ status: 'error', message: err.message }), 'utf-8');
        log.error('vynetconfig.json save error:', err.message);
      }
    });
    return;
  }
